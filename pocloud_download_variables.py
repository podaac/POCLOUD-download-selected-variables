...
def create_parser():
    ...
    # variable arguments
    parser.add_argument("-v", "--variable", dest="var",
                        help="The list of variables to be downloaded, please use the -v=\"var1,var2,var3\" syntax when calling from the command line. Default: \"*\".",
                        default="*")
...
def run(args=None):
    ...
    if len(downloads_data) < 1:
      print("\n\n*** Could not find files to download, please try again ! ***\n\n")
      exit()
    ...
    for v in ncin.variables:
      for i in range(len(avar)):
        if(v == avar[i]):
          ...
          for i in range(len(ncin.variables[v].dimensions)):
            #print(ncin.variables[v].dimensions[i])
            #print(ncin.variables[v].shape[i])
